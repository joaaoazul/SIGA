...
const AthleteSetup = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // O Supabase tem um trigger que cria automaticamente o perfil
      await new Promise(resolve => setTimeout(resolve, 2000));

      // 3. Atualizar o perfil existente (criado pelo trigger)
      const { error: profileError } = await supabase
        ...
        .update({
          ...
          trainer_id: invite.trainer_id, // Associar ao trainer
          setup_complete: true,
      ...
      if (!signInError) {
        ...
        // Pequeno delay para garantir que o auth state é atualizado
        setTimeout(() => {
          navigate('/');
        }, 1000);
      } else {
  ...
  // Componentes de cada Step (mantém os mesmos do código anterior)
  const Step1 = () => (
